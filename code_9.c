...
CommandStatus evaluate_sleep(Spreadsheet* sheet, Cell* cell, const char* expr) {
    char* closing_paren = strchr(expr, ')');
    if (!closing_paren || closing_paren == expr + 6) {
        cell->error_state = 1;
        return CMD_UNRECOGNIZED;
    }
    
    // Extract argument between parentheses
    size_t arg_len = closing_paren - (expr + 6);
    char* arg = malloc(arg_len + 1);
    strncpy(arg, expr + 6, arg_len);
    arg[arg_len] = '\0';

    int duration = evaluate_cell_reference(sheet, arg, cell);
    free(arg);

    pthread_t thread;
    int* arg_ptr = malloc(sizeof(int));
    *arg_ptr = duration;
    if (pthread_create(&thread, NULL, sleep_wrapper, arg_ptr) != 0) {
        free(arg_ptr);
        cell->error_state = 1;
        return CMD_UNRECOGNIZED;
    }
