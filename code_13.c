...
typedef struct Cell {
    ...
    char* formula;
    int error_state;
    int row;    // cell’s row index
    int col;    // cell’s column index
    struct Cell** parents;
    int parents_count;
...
CommandStatus set_cell_value(Spreadsheet* sheet, int row, int col, const char* expr, double* sleep_time);
...
void add_parent(Cell* child, Cell* parent) {
    ...
    if (child->parents_capacity == 0) {
        child->parents_capacity = 4; // initial capacity set to a small number
