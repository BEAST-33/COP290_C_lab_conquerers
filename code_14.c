...
// Updated Cell structure: row and col are now of type short.
typedef struct Cell {
    char* formula;
...
// Spreadsheet structure: rows and cols are stored as short.
typedef struct {
    Cell*** grid;
    short rows;
    short cols;
...
// Range structure: row and column indices as short.
typedef struct {
    short start_row;
    short start_col;
    short end_row;
...
// Function prototypes for cell operations and parsing:
...
/* ----- Helper: Linear index for a cell ----- */
static inline int cell_index(Spreadsheet* sheet, short row, short col) {
...
/* ----- parse_cell_reference: now uses short pointers ----- */
void parse_cell_reference(const char* cell, short* row, short* col) {
    char col_name[4] = {0};
    int i = 0;
...
// get_column_name and column_name_to_number remain unchanged.
char* get_column_name(int col) {
...
// 2. Special handling for SLEEP() functions.
// If the expression begins with "SLEEP(", then process its argument.
...
CommandStatus evaluate_formula(Spreadsheet* sheet, Cell* cell, const char* expr, double* sleep_time) {
    ...
    if (strncmp(expr, "SUM(", 4) == 0 || strncmp(expr, "AVG(", 4) == 0 ||
        ...
        // Iterate over the cells in the range. For each one, add as dependency and aggregate values.
        ...
        for (int r = range.start_row; r <= range.end_row; r++) {
            for (int c = range.start_col; c <= range.end_col; c++) {
                ...
                add_parent(cell, ref_cell);  // Add dependency: each cell in the range is a parent.
...
/* ----- create_spreadsheet: rows and cols now passed as short ----- */
