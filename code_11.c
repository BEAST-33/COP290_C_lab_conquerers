...
Cell** topological_sort(Cell** cells, int count, int* sorted_count) {
    *sorted_count = 0;
    ...
    // Build adjacency matrix and calculate in-degrees
    for (int i = 0; i < count; i++) {
        Cell* cell = cells[i];
        for (int j = 0; j < cell->dependent_count; j++) {
            Cell* dependent = cell->dependents[j];
            // Find index of dependent in cells array
            for (int k = 0; k < count; k++) {
                if (cells[k] == dependent) {
                    adj[i][adj_count[i]++] = k;
                    in_degree[k]++;
                    break;
                }
    ...
    while (front < rear) {
        ...
        sorted[(*sorted_count)++] = cells[node];
...
void propagate_changes(Spreadsheet* sheet, Cell* modified_cell) {
    ...
    if (count <= 1) {  // Only the modified cell itself
        free(dependents);
        return;
    }
    ...
    // Skip the first cell (the modified cell itself)
    for (int i = 1; i < sorted_count; i++) {
        ...
        if (cell->formula) {
            ...
            // If value didn't change, we can stop propagation along this path
            if (old_value == cell->value && !cell->error_state) {
                continue;
            }
        }
    }
